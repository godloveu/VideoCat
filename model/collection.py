...
class collectionThread(QThread):
    ...
    def run(self):
        ...
        # 获取用户id
        self.user_id = self.url.split('collection/')[1].split('?')[0]
        # 获取代理ip
        ip = get_ip()
        # ip数量
        ip_count = len(ip)
        ...
        while True:
            ...
            api = f'https://www.iesdouyin.com/web/api/mix/item/list/?reflow_source=reflow_page&mix_id={self.user_id}&count=10&cursor={self.cursor}'
            try:
            ...
            for aweme in aweme_list:
                ...
                if self.username is None:
                    ...
                    self.collection = aweme['mix_info']['mix_name']
                    self.info.emit(self.username, self.collection)
